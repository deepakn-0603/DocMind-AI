...
from app.config import settings  # ✅ Correct
...
def get_embeddings():
    ...
    #     logger.debug("Initializing Hugging Face embeddings client")
    logger.debug("Initializing local Hugging Face embeddings")
    return HuggingFaceEmbeddings(
        model_name=settings.HUGGINGFACE_EMBEDDING_MODEL,
        encode_kwargs={'normalize_embeddings': True}
    )
    
    # # Option 2: Use local Hugging Face embeddings (no API key needed)
    # else:
    #     logger.debug("Initializing local Hugging Face embeddings")
    #     return HuggingFaceEmbeddings(
    #         model_name=settings.HUGGINGFACE_EMBEDDING_MODEL,
